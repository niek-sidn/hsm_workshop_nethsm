...
after it is up:  
`ssh -l root -o UserKnownHostsFile=/dev/null -o "StrictHostKeyChecking no" -o "LogLevel ERROR" 192.168.13.13  #(use actual pub ip4)`
...
`export NETHSM_PUB_IPV4="192.168.13.13" #(use actual pub ip4)`  
...
after it is up:  
`ssh -l root -o UserKnownHostsFile=/dev/null -o "StrictHostKeyChecking no" -o "LogLevel ERROR" 123.123.123.123  #(use actual pub ip4)`

`apt-get update && apt-get upgrade -y && apt-get install -y git ca-certificates curl pwgen \`  
...
`export NETHSM_PUB_IPV4="192.168.13.13" #(use actual pub ip4 of the *nethsm* host)`  
`envsubst '$NETHSM_PUB_IPV4' < hsm_workshop_nethsm/files/p11nethsm.conf.envsubst > hsm_workshop_nethsm/files/p11nethsm.conf`  
`NOTE: if you changed the passwords in hsm_env_vars.envsubst on the nethsm host, also: vim files/p11nethsm.conf`  
...
`incus file push /root/hsm_workshop_nethsm/files/p11nethsm.conf user${X}/usr/local/etc/nitrokey/p11nethsm.conf`  
